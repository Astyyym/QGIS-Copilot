...
- 阶段：增量版本 v2.0，Goal 8 已由哥哥确认验收通过
- 状态：Goal 6、Goal 7、Goal 8 均已通过；Goal 8 具有自动化、空间预览和筛选导出 Desktop 证据，裁剪成功路径的最终验收以哥哥确认记录为准
- 负责人：哥哥与小鱼共同确认
- 当前进入条件：Goal 6 已由哥哥确认通过；Goal 7 代码与独立 QGIS 自动化已通过，真实 Desktop 验收仍是当前阶段门
...
- [x] Goal 8：空间关系预览、裁剪与筛选导出闭环（哥哥确认通过）
  ...
  - [x] 确认式 `export_filtered_features`（Desktop：表达式 `"省份" = '四川'` 先验证后确认，生成 `D:\Temp\qgis_copilot_filtered.gpkg` 与“四川气象站点”35 个要素，源图层未覆盖）
