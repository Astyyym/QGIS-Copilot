...
- 阶段：Goal 4——完成安全的 GIS 执行闭环
- 状态：代码与 QGIS 自带 Python Processing 回归通过；真实 QGIS Desktop 的计划/取消/确认/失败四条证据待验收
- 负责人：哥哥与小鱼共同确认
...
- [ ] Goal 2：接入模型并建立 Agent 基础循环（代码与自动化验证通过；真实 QGIS 面板配置/真实外部模型端到端验收阻塞）
  ...
  - [ ] 真实 QGIS 面板中保存凭据、连通性与真实外部模型请求：需要哥哥提供可用的测试端点和 API Key，并由哥哥亲自输入（密钥不得由小鱼接收或键入）
...
- 不修改已有 QGIS MCP 仓库；
- 不接入真实模型、Agent 工具、Processing 写入或 MCP；
- 不安装外部依赖；
- 不创建或推送远程仓库；
- 不发布 QGIS 插件。
