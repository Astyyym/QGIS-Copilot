...
- Goal 10 优先新增 `tools/raster/` 下的诊断、provider 探测、计划和验证模块，以及独立 `tasks/raster_processing.py`；现有矢量文件只允许最小兼容适配，不在 Goal 10 迁移。
- 每个 v3 Goal 必须运行 Goal 1-9 固定回归矩阵和真实 Desktop 旧代表路径。旧功能回归时当前 Goal 立即失败并停止扩展，不能通过修改旧测试预期或降低断言宣布通过。
